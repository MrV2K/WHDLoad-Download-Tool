...
Procedure Scan_FTP()
  ...
  If OpenFTPEx(#FTP,#PBEx_FTP_Protocol_FTP,FTP_Server,FTP_Port,FTP_User,FTP_Pass,#PB_Ascii,@ErrorOutput$)
    ...
    Repeat
      ...
      SetGadgetText(#INFO_TEXT,"Opening FTP Folder - "+FTP_Folder+" Attempt:"+Str(Count))
      Delay(100)
      Count+1
      While WindowEvent() : Wend
      GetFTPDirectoryEx(#FTP,@FileName$,@ErrorOutput$)
    Until RemoveString(FileName$,"/")=FTP_Folder Or Count=250
    
    If result>-1   
      If ExamineFTPDirectoryEx(#FTP,@ErrorOutput$)
        SetGadgetText(#INFO_TEXT,"Reading FTP Folder - "+FTP_Folder)
        Delay(500)
              ...
              If FindString(FileName$,"Commodore Amiga - WHDLoad - Games") Or FindString(FileName$,"Commodore Amiga - WHDLoad - Demos")
                If FTPDirectoryEntrySizeEx(#FTP,@ErrorOutput$)>0
                  ...
                  If FileSize(Dat_Folder+FileName$)=-1 ; If the dat file doesn't exist, download it
                    SetGadgetText(#INFO_TEXT,"Downloading : "+FileName$)
  ...
  If Trim(ErrorOutput$)<>""
    If CreateFile(log_file, Home_Path+"ftp.log")
      WriteString(log_file, ErrorOutput$)
      CloseFile(log_file)  
    EndIf
  EndIf
        ...
        If SetFTPDirectoryEx(#FTP,FTP_Folder,@ErrorOutput$) 
          ...
          ForEach Down_List() 
            ...
            If ReceiveFTPFileEx(#FTP,Down_List()\Down_Name, down_path,#False,@ErrorOutput$)
             PrintN("Downloading : " + Down_List()\Down_Name)              
