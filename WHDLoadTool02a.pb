...
Global NewList Game_List.File_Data()
...
Procedure Disable_Gadgets(bool.b)
  ...
  DisableGadget(#LIST_ADD_BUTTON,bool)
...
Procedure Filter_List()
  ...
  ForEach Game_List()
    If Game_List()\File_Filtered<>#True : AddElement(Filtered_List()) : Filtered_List()=ListIndex(Game_List()) : EndIf
  Next
...
Procedure Load_List()
  ...
  If result<>#PB_MessageRequester_No
    ...
    If ListSize(List_Games())>0
      SetGadgetText(#LIST_ADD_BUTTON,"Edit List")
...
Procedure Edit_List()
  ...
  If result<>#PB_MessageRequester_No
    ...
    ForEach Game_List()
      ...
      SetGadgetItemState(#EDIT_LIST,ListIndex(Game_List()),#PB_ListIcon_Checked)
    Next
          ...
          If changes
            ...
            Select result
              Case #PB_MessageRequester_Yes
                ...
                DisableGadget(#CLEAR_EDITS_BUTTON,#False)
                ForEach Game_List()
                  If GetGadgetItemState(#EDIT_LIST,ListIndex(Game_List()))=#PB_ListIcon_Checked
                    Game_List()\File_Ignore=#False
                  Else          
                    Game_List()\File_Ignore=#True
                  EndIf
...
Procedure Scan_FTP()
  ...
  OpenWindow(#INFO_WINDOW,0,0,600,30,"Checking for updates...",#PB_Window_Tool|#PB_Window_WindowCentered,WindowID(#MAIN_WINDOW))
  
  oldgadgetlist=UseGadgetList(WindowID(#INFO_WINDOW))
  
  TextGadget(#INFO_TEXT,0,5,600,25,"Connecting to "+ftp_server+" on port:"+Str(ftp_port),#PB_Text_Center)
  
  While WindowEvent() : Wend

  If OpenFTP(#FTP,FTP_Server,FTP_User,FTP_Pass,FTP_Passive,FTP_Port)
    SetGadgetText(#INFO_TEXT,"Connected to "+ftp_server+" on port:"+Str(FTP_Port))
    Delay(500)
    If SetFTPDirectory(#FTP,FTP_Folder)    
            ...
            If FTPDirectoryEntryType(#FTP)=#PB_FTP_File
              If FindString(FTPDirectoryEntryName(#FTP),"Commodore Amiga - WHDLoad - Games") Or FindString(FTPDirectoryEntryName(#FTP),"Commodore Amiga - WHDLoad - Demos")
                If FTPDirectoryEntrySize(#FTP)>0
                  ...
                  If FileSize(Dat_Folder+FTPDirectoryEntryName(#FTP))=-1 ; If the dat file doesn't exist, download it 
                    SetGadgetText(#INFO_TEXT,"Downloading : "+FTPDirectoryEntryName(#FTP))
                    ReceiveFTPFile(#FTP,FTPDirectoryEntryName(#FTP),Dat_Folder+FTPDirectoryEntryName(#FTP))
                    new_dat=#True
                  EndIf
                  AddElement(Dat_List())
                  Dat_List()=Dat_Folder+FTPDirectoryEntryName(#FTP) ; Add file to downloaded file list
                EndIf
              EndIf
            EndIf
          EndIf
      ...
      EndIf
    Else
      MessageRequester("Error","Cannot find FTP folder!",#PB_MessageRequester_Error|#PB_MessageRequester_Ok)
    EndIf
    CloseFTP(#FTP)        
  Else
    SetGadgetColor(#INFO_TEXT,#PB_Gadget_FrontColor,#Red)
    SetGadgetText(#INFO_TEXT,"Error: Cannot connect to FTP!")
    Delay(1000)
    Goto Proc_Exit
  EndIf
  
  If ListSize(Dat_List())=0
    SetGadgetText(#INFO_TEXT,"Scanning local dat folder.")
  ...
  If ListSize(Dat_List())>0  
    ...
    SetGadgetText(#INFO_TEXT,"Processing dat files...")
    ...
    ForEach File_List() ; compare the file list to the dat folder map and delete anythin not needed
  ...
  Else
    ...
    MessageRequester("Error","Nothing downloaded!"+Chr(10)+"Please check the FTP status.",#PB_MessageRequester_Error|#PB_MessageRequester_Ok)
  ...
  If new_dat : MessageRequester("New dat file", "Dat file downloaded.",#PB_MessageRequester_Ok|#PB_MessageRequester_Info) : EndIf
...
Procedure Download_FTP()
  ...
  If ListSize(Down_List())>0   
    ...
    If MessageRequester("Warning","Download "+Str(ListSize(Down_List()))+" Files?",#PB_MessageRequester_YesNo|#PB_MessageRequester_Warning)=#PB_MessageRequester_Yes
      ...
      OpenWindow(#INFO_WINDOW,0,0,600,30,"FTP Download",#PB_Window_Tool|#PB_Window_WindowCentered,WindowID(#MAIN_WINDOW))
      
      oldgadgetlist=UseGadgetList(WindowID(#INFO_WINDOW))
      
      TextGadget(#INFO_TEXT,0,5,540,25,"Connecting To "+ftp_server+" on port:"+Str(ftp_port),#PB_Text_Center)
      ButtonGadget(#INFO_BUTTON,545,2,50,25,"Cancel")
      ...
      If OpenFTP(#FTP,ftp_server,ftp_user,ftp_pass,ftp_passive,ftp_port)
        ...
        If SetFTPDirectory(#FTP,FTP_Folder)   
          ForEach Down_List()
            If WaitWindowEvent(100) 
              If EventGadget()=#INFO_BUTTON 
                If MessageRequester("Warning","Cancel download?",#PB_MessageRequester_YesNo|#PB_MessageRequester_Warning)=#PB_MessageRequester_Yes
                  SetGadgetText(#INFO_TEXT,"*** FTP download cancelled by user ***")
                  Delay(2000)
                  ftp_log+"*** FTP download cancelled by user ***"+#CRLF$
                  Break
                Else
                  While WindowEvent() : Wend
                EndIf
            ...
            SetGadgetText(#INFO_TEXT,"Downloading : " + Down_List()\Down_Name)
            If Use_Subfolder
            ...
            EndIf          
            If ReceiveFTPFile(#FTP,Down_List()\Down_Name, down_path)
...
Procedure Scrape_Data()
  ...
  SortStructuredList(Game_List(),#PB_Sort_Ascending|#PB_Sort_NoCase,OffsetOf(File_Data\File_Name),TypeOf(file_Data\File_Name))
...
Procedure About_Window()
  ...
  Repeat
    ...
    If EventGadget()=#ABOUT_LINK And EventType()=#PB_EventType_LeftClick
      RunProgram("https://www.paypal.com/donate/?token=-kZNJMLMeKb6rG_Lij2dBrK3SaHKXXKt7pdFdf71y5xF5WxfCcE2g5UlkUo77V3BbXDtgqKVCNIArqV0&locale.x=US","","")
    EndIf
...
Procedure Help_Window()
  ...
  output$+"---------"+#CRLF$
  output$+"The file list shows a list of the software available on the FTP site. "
  ...
  output$+"FTP Settings"+#CRLF$
  output$+"------------"+#CRLF$
  output$+"This section shows the current FTP settings. A couple of text boxes are blocked on purpose as they need to remain unchanged. "
  output$+"There are two entries that you can change. These are... "+#CRLF$
  output$+#CRLF$
  ...
  output$+#CRLF$
  output$+"Folder Paths"+#CRLF$
  output$+"------------"+#CRLF$
  output$+"This section lets you set the paths that the archives are downloaded to. "
  output$+"By default this tools supports three types of archives (games, demos and beta). Beta files are internally classed as games so are stored in the games folder. "
  output$+"If you want to download all the files into the same subfolder, you can set both boxes to the same path. "+#CRLF$
  output$+#CRLF$
  output$+"The 'Set' button on the 'Parent' path will open a requester so you can choose the default download path. *Note* You cannot set this path as a root folder for safety reasons. "
  output$+"The 'Games' and 'Demos' boxes are for the names of the subdirectories that the categories will be saved to. "
  output$+"You do not need to enter a full path in this box as it's only used as the subfolder name. "+#CRLF$
  output$+#CRLF$
  output$+"If you select the 'Use Subfolders' check box, any files you download will be separated into the folder names specified in the 'Games' and 'Demos' boxes. "+#CRLF$
  output$+#CRLF$
  output$+"To set a subfolder path, press simply type the name of the folder into one of the boxes. You can only use these boxes if the 'Use Subfolders' check box is selected. "+#CRLF$
  output$+#CRLF$
  ...
  output$+#CRLF$
  output$+"The 'Archive Type' section lets you set whether to show games or demos. Beta files can be filtered in the 'System' section but are stored in the games folder (if selected)."+#CRLF$
  output$+#CRLF$
  ...
  output$+"Lists"+#CRLF$
  output$+"-----"+#CRLF$ 
  output$+"  Create List  - Opens a window with all the files in the database"+#CRLF$
  ...
  output$+"                 options, overwrite this file, else use your own name."+#CRLF$
  output$+#CRLF$
  ...
  output$+"  4. Press the 'Download' button to download the files from the FTP."+#CRLF$
  output$+"  5. If you want to save your settings, press the 'Save Prefs' button"+#CRLF$
  ...
  output$+"====="+#CRLF$
  output$+#CRLF$
  output$+"Lists are a quick way to create and manage curated sets of WHDLoad files. Loading or creating lists will reset any changes that have been made to the filter as it can hide files. Once loaded, lists can be filtered, but I would suggest using the edit window instead."+#CRLF$
  output$+#CRLF$
  ...
  output$+"-------------"+#CRLF$ 
  output$+"  1. Press the 'Load Data' button to load the main database."+#CRLF$
  output$+"  2. Open the 'Create List' window and add or remove more files."+#CRLF$
  ...
  output$+"The list files are just text files with lists of filenames in them. If you want to add comments to a list file, you can add them by using a semicolon ';' at the beginning of the text line. You can edit list files in any text editor and add comments but remember to save them with the '.lst' file extension."+#CRLF$
  output$+#CRLF$+#CRLF$
...
Procedure Main_Window()
  ...
  OpenWindow(#MAIN_WINDOW,5,5,875,577,"",#PB_Window_ScreenCentered|#PB_Window_SystemMenu)
  ...
  ListIconGadget(#MAIN_LIST,5,5,445,545,"",0,#PB_ListIcon_FullRowSelect | #LVS_NOCOLUMNHEADER)
  ...
  FrameGadget(#PB_Any,455,150,320,150,"Save Folders")
  
  TextGadget(#PB_Any,465,175,40,25,"Parent")
  StringGadget(#WHD_MAIN_STRING,510,172,162,25,WHD_Folder,#PB_String_ReadOnly)
  ButtonGadget(#WHD_OPEN_PATH_BUTTON,682,172,40,25,"Open")
  ButtonGadget(#WHD_SET_PATH_BUTTON,727,172,40,25,"Set")
  TextGadget(#PB_Any,465,205,40,25,"Games")
  StringGadget(#WHD_GAME_STRING,510,202,212,25,WHD_Game_Folder)
  ButtonGadget(#WHD_OPEN_GAME_BUTTON,727,202,40,25,"Open")
  TextGadget(#PB_Any,465,235,40,25,"Demos")
  StringGadget(#WHD_DEMO_STRING,510,232,212,25,WHD_Demo_Folder)
  ButtonGadget(#WHD_OPEN_DEMO_BUTTON,727,232,40,25,"Open")
  CheckBoxGadget(#WHD_SUBFOLDER_CHECK,465,265,140,25,"Use Subfolders")
  SetGadgetState(#WHD_SUBFOLDER_CHECK,Use_Subfolder)
  CheckBoxGadget(#WHD_0TOZ_CHECK,615,265,140,25,"Add 0-Z Subfolders")
  ...
  FrameGadget(#PB_Any,455,300,320,250,"Filter")
  
  FrameGadget(#PB_Any,460,315,150,40,"Archive Type")
  
  CheckBoxGadget(#GAME_OPTION,465,330,60,23,"Games")
  CheckBoxGadget(#DEMO_OPTION,530,330,60,23,"Demos")
  
  FrameGadget(#PB_Any,460,355,150,95,"System")
  
  CheckBoxGadget(#AMIGA_OPTION,465,370,60,25,"Amiga")
  CheckBoxGadget(#ARCADIA_OPTION,530,370,60,25,"Arcadia")
  CheckBoxGadget(#CD32_OPTION,465,395,60,25,"CD32")
  CheckBoxGadget(#CDTV_OPTION,530,395,60,25,"CDTV")
  CheckBoxGadget(#CDROM_OPTION,465,420,60,25,"CDROM")
  CheckBoxGadget(#BETA_OPTION,530,420,60,25,"Beta")
  
  FrameGadget(#PB_Any,460,450,150,95,"Extras")
  
  CheckBoxGadget(#AGA_OPTION,465,470,60,20,"AGA")
  CheckBoxGadget(#ECS_OPTION,530,470,65,20,"ECS/OCS")
  CheckBoxGadget(#NTSC_OPTION,465,495,60,20,"NTSC")
  CheckBoxGadget(#PAL_OPTION,530,495,60,20,"PAL")  
  CheckBoxGadget(#FILES_OPTION,465,520,60,20,"Files")
  CheckBoxGadget(#IMAGE_OPTION,530,520,60,20,"Image")
  
  FrameGadget(#PB_Any,615,315,155,200,"Language")
  
  CheckBoxGadget(#CZECH_OPTION,625,335,60,25,"Czech")
  CheckBoxGadget(#GREEK_OPTION,700,335,60,25,"Greek")
  CheckBoxGadget(#DANISH_OPTION,625,360,60,25,"Danish") 
  CheckBoxGadget(#ITALIAN_OPTION,700,360,60,25,"Italian")
  CheckBoxGadget(#DUTCH_OPTION,625,385,60,25,"Dutch")
  CheckBoxGadget(#MULTI_OPTION,700,385,60,25,"Multi")
  CheckBoxGadget(#ENGLISH_OPTION,625,410,60,25,"English")
  CheckBoxGadget(#POLISH_OPTION,700,410,60,25,"Polish")
  CheckBoxGadget(#FINNISH_OPTION,625,435,60,25,"Finnish") 
  CheckBoxGadget(#SPANISH_OPTION,700,435,60,25,"Spanish")
  CheckBoxGadget(#FRENCH_OPTION,625,460,60,25,"French")
  CheckBoxGadget(#SWEDISH_OPTION,700,460,60,25,"Swedish")
  CheckBoxGadget(#GERMAN_OPTION,625,485,60,25,"German")
  ButtonGadget(#CLEAR_LANG_BUTTON,700,486,65,22,"Clear")

  
  ButtonGadget(#CLEAR_BUTTON,615,518,78,27,"Clear Filter")
  ButtonGadget(#RESET_BUTTON,695,518,75,27,"Reset Filter")   
  ...
  ButtonGadget(#LIST_ADD_BUTTON,785,115,80,30,"Create List")  
  ...
  FrameGadget(#PB_Any,780,390,90,160,"Misc")
  
  ButtonGadget(#SAVE_BUTTON,785,410,80,30,"Save Prefs")
  ButtonGadget(#LOAD_BUTTON,785,445,80,30,"Load Prefs")   
  ...
  Select Gadget
    ...
    Case #CLEAR_EDITS_BUTTON
      If MessageRequester("Warning","Remove all edits and list data?",#PB_MessageRequester_Warning|#PB_MessageRequester_YesNo)=#PB_MessageRequester_Yes
        ...
        Next
        DisableGadget(#CLEAR_EDITS_BUTTON,#True)
        SetGadgetText(#LIST_ADD_BUTTON,"Create List")
        Set_List_Gadgets(#True)
        Draw_List()
      EndIf
    ...
    Case #LOAD_BUTTON
      Path=OpenFileRequester("Load Prefs",Home_Path,"Prefs File (*.prefs)",0)
      If Path<>""
        Load_Prefs(Path)
        SetGadgetText(#FTP_USER_STRING,ftp_user)
        SetGadgetText(#FTP_PASS_STRING,ftp_pass)
    ...
    Case #SAVE_BUTTON
      Path=OpenFileRequester("Save Prefs",Home_Path,"Prefs File (*.prefs)",0)
      If Path<>""
...
; Compiler = PureBasic 6.00 Beta 1 - C Backend (Windows - x86)
; Debugger = Standalone